    ...
    <div className="flex flex-col md:flex-row w-full gap-10">
      <div className="flex flex-row md:flex-col w-full md:w-1/2 gap-6 justify-start">
        {gymBuddiesBenefits.slice(0, 2).map((benefit) => (
          <div key={benefit.id} className="flex flex-col">
            <div className="flex items-center justify-center bg-yellow-500 rounded-full w-6 h-6 text-white text-xs md:text-sm">
              <p>{`0${benefit.id}`}</p>
            </div>
            <p className="text-md md:text-lg font-bold py-2">{benefit.title}</p>
            <p className="text-xs md:text-sm text-gray-400">
              {benefit.description}
            </p>

            <div className="flex flex-row items-center gap-2 text-white capitalize mt-2 text-xs md:text-sm">
              <p>learn more</p>
      ...
      <div className="w-full md:w-[70%] flex items-center justify-center">
        <Image
          ...
          width={1000}
          height={1000}
          className="w-full md:w-2/3 h-80 md:h-96 object-cover rounded-lg"
          priority
        />
      </div>
